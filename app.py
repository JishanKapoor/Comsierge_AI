                ...
                if forwarding_rule['forward_to'] != from_number:
                    if mode_value == 'dnd' and forwarding_rule['forward_to'] == user.get('personal_phone'):
                        ...
                        if rule_priority_mode == 'all':
                            ...
                            rule_should_forward = True
                        elif rule_priority_mode == 'high' and priority == 'high' and (not forwarding_rule.get('high_filters') or high_type in forwarding_rule['high_filters']):
                            rule_should_forward = True

                        if rule_should_forward:
                            message_to_forward = translated_msg if forwarding_rule['forward_to'] == user.get('personal_phone') else incoming_msg
                            ...
                            logging.info(f"Forwarded message to {forwarding_rule['forward_to']} (Message ID: {forwarded_msg.sid})")
        except Exception as e:
