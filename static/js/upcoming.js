document.addEventListener('DOMContentLoaded', () => {
    ...
    function sortGroups() {
        ...
        const groupItems = Array.from(eventList.children).filter(child => child.classList.contains('group-item'));
        groupItems.sort((a, b) => {
        ...
        eventList.innerHTML = '';
        groupItems.forEach(g => eventList.appendChild(g));
        if (groupItems.length === 0) {
            const noEventsMsg = document.createElement('p');
            noEventsMsg.id = 'no-events-msg';
            noEventsMsg.className = 'text-xs text-zinc-400 text-center';
